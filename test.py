...
# Load the CSV data
microwave_link_data = pd.read_csv(
    "C:\\Users\\user\\Documents\\UNI\\Thesis\\Microwave_link.csv"
)
...
if "tanggal" not in microwave_link_data.columns:
    print(
        "Error: 'tanggal' column is missing from Microwave Link data. Can't be calculated."
    )
else:
    ...
    merged_data = pd.merge(
        microwave_link_data,
        sims_data,
        left_on="curr_lic_num",
        right_on="curr_lic_num",
        how="left",
    )
...
X = merged_data.drop(columns=["curr_lic_num"])
...
mismatched_data = microwave_link_data[
    ~microwave_link_data["curr_lic_num"].isin(sims_data["curr_lic_num"])
]
...
# # Assuming 'category_column' is a categorical column in mismatched data
